...
设置页「系统 · 运行环境」列出全部要用到的外部程序，每条报**它在哪**（不只是有没有）：

| | 用在哪 | 缺了会怎样 |
|---|---|---|
| bash | 模型执行命令 | `run_command` 整个不注册 |
| git | 版本面板 | 读不到分支、改动与差异 |
| ripgrep | 全文搜索 | 只是慢——内置遍历顶上 |
| Node.js | 插件运行时 | 插件跑不了 |

前两条缺了标「需要安装」，后两条标「未安装（可选）」。Windows 上装了 winget 的话
每条缺失项带一个按钮，点了会开一个终端窗口跑 `winget install`。
**装完要重启 qywork**——当前进程的 `PATH` 是启动时的快照，看不到新装的程序。
...
技能、记忆、MCP 都有**三层**：内置（随程序发布，只读）> 项目（工作区
`.agents/`，别的 CLI 也读得到）> 全局（`~/.qywork/`），同名先认领的赢。
**插件不分层**，只有 `~/.qywork/plugins/` 一份——它贡献的是这个 agent 的能力，
不是某个仓库的内容。逐条开关在对话右侧面板里，**只影响当前那一条会话**。
...
压缩前的 213%——单测能测「触发了压缩」，测不出「压缩之后模型还记不记得」。
