 ...
 *
 * 两个子命令共用 WSL 文件系统上的同一份检出 `~/qywork-wsl`：从本仓库的 git 对象库取 `ref`
 * （缺省为本检出的 HEAD），检出到本地分支 `wsl`，再 `bun install --frozen-lockfile`。
 * 只带已提交的内容，工作区里未提交的改动不在其中。检出不放在 `/mnt/c` 下：那里走 9p，I/O 慢，
 * 大小写与权限语义也与 Linux 文件系统不同。
 *
 * `run` 在新起的 Xvfb 显示与 `dbus-run-session` 会话里执行命令，AT-SPI 总线由会话总线上的
 * `org.a11y.Bus` 按需激活。不用 WSLg：它的显示与会话总线由所有 WSL 进程共用，窗口出现在
 * Windows 桌面上，AT-SPI 注册表里也列着别的进程的应用。
 *
 * 工具链需事先装进 WSL：`.github/actions/setup-build/action.yml` 的 apt 清单，加 pkg-config、
 * xvfb、xauth，rustup 的 stable，`~/.bun` 下与 `packageManager` 同版本的 Linux 版 bun。
 * 另装 nodejs 与 python-is-python3：CI 的 runner 镜像自带 `node` 与 `python`，测试依赖二者。
 *
...
/**
 * WSL 里执行的前置步骤，`$1` 为 git 对象库的 Windows 路径，`$2` 为提交号，其余参数原样 `exec`。
 *
...
const PREPARE = [
  ...
  'src=$(wslpath -u "$1"); sha=$2; shift 2',
  'git init -q "$HOME/qywork-wsl"',
  'cd "$HOME/qywork-wsl"',
  'git fetch -q --no-tags "$src" "$sha"',
  'git checkout -q --force -B wsl "$sha"',
  'git clean -q -fd',
  'echo "qywork-wsl: $(git log -1 --format=\'%h %s\')"',
...
const GUI = [
  ...
  'rt=$(mktemp -d)',
  'trap \'rm -rf "$rt"\' EXIT',
  'XDG_RUNTIME_DIR=$rt xvfb-run -a -s "-screen 0 1920x1080x24" dbus-run-session -- "$@"',
].join('\n')
