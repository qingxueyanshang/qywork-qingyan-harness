 ...
 * ## 同一个错误犯了第二次：这次是「起服务器」
 *
 * softDeny 里曾经还有三条：「启动进程或服务器并让它一直占着资源」「读取或遍历
 * 工作区之外的路径（哪怕只是列文件名）」「安装依赖清单里没有的新包」。
 * 它们和上面那个 `node a.js` 是同一类误判——把编码 agent 的本职工作当成风险。
 *
 * 实测后果：用户让模型做一个网页游戏，模型起本地服务器验证页面，被拒。而
 * `allow` 第一条明明写着「跑脚本…是本职工作」——两条规则对 `npm run dev`
 * 给出相反答案。更糟的是判定还不稳定：同一条命令连跑两次会给出相反结论，
 * 因为它是概率判断（`policy.ts` 早就为 `$HOME` 那条记过同样的事）。
 *
 * 对照三个开源实现（`view-agent/` 下，用户点名要求核对）：
 *
 * - **pi**：没有任何命令内容裁决。SECURITY.md 原话是它跑在「运行它的那个用户的
 *   安全边界内」，本地账号与该账号可写的文件**与 agent 进程同属一个信任边界**，
 *   隔离该由用户拿容器 / 虚拟机去做；真正的边界是**操作系统权限边界**，
 *   不是「这条命令看起来危不危险」。
 * - **prime-agent**：bash 工具只有 `command` + `timeout`，无裁决。
 * - **cc-haha**：有 `bashSecurity.ts`，但拦的是 `$()` / `<()` / zsh 扩展这类
 *   **语法绕过**——防的是用户写的权限规则被技巧绕开；它那个 LLM 分类器
 *   (`bashClassifier.ts`) 匹配的是**用户自己写的自然语言规则**，
 *   而且外部构建里直接 `return false` 关掉。
 *
 * 三家都没有「LLM 逐条判这条命令危不危险」这个东西。
 *
 * 所以 softDeny 收敛到三条，判据是**不可逆、越界、或把东西送出去**：
 * 成片删工作区（误删代价高且难还原）、push / 改写 git 历史（对外不可逆）、
 * 把工作区内容发到网络上（数据外泄）。其余交回 allow——在这台机器上，
 * agent 能做的事用户本人也能做。
 *
...
export interface ClassifyInput {
   ...
   */
  timeoutMs?: number
  /* 注意措辞：这条事实只能说「能跑多久」，不能让人推出「所以没事」。
     实测踩过——第一版写成「（只允许本机回环，工具层强制）」，分类器据此
     推出 `python -m http.server` 的服务「不对外提供服务」，而它绑的是 0.0.0.0。
     把真事实写成会推出假结论的形状，比不给这条事实更糟。 */
  /**
...
function buildUserMessage(input: ClassifyInput, stage: 1 | 2): string {
  ...
  return [
    ...
    ...(input.timeoutMs === undefined
      ? []
      : [
          `<fact>这条命令会在 ${input.timeoutMs} 毫秒后被强制终止。` +
            `**这只约束它能跑多久，不改变它做什么**：它在这段时间里监听哪个网卡、` +
            `对谁可见、写什么、发什么请求，都由命令自己决定。</fact>`,
        ]),
    ...(input.probeUrl === undefined
      ? []
      : [
          `<fact>这次调用带了探测地址 ${input.probeUrl}，工具的动作是：` +
            `起进程 → 轮询该地址至就绪 → 抓一次响应 → 立即杀掉整棵进程树，` +
            `进程不会活过这次调用。
` +
            `**这一条只约束进程存活多久，不改变命令本身做什么**：` +
            `它在存活期间监听哪个网卡、对谁可见、写什么、发什么请求，都由命令自己决定。` +
            `探测地址限定为回环，管的是「我们去连谁」，与服务器绑在哪无关。</fact>`,
        ]),
