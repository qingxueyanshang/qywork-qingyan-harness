 ...
 *
 * 上一版是「静态允许清单 + 组合结构检测 + 判不了就交给 LLM 分类器」。三件事都删了：
 *
 * - **允许清单**（十几条只读命令）的存在只是为了省一次分类器往返。分类器没了，
 *   它也就没有了理由；而它带来的复杂度（参数守卫、前缀守卫、组合符号扫描、
 *   可疑码点检测）全都是为「能不能安全地跳过分类器」服务的。
 * - **LLM 分类器**被删的理由是它不稳定：同一条命令连跑两次给出相反结论，实测两次。
 *   而上面三类拦截**全部能用确定性规则表达**，不该交给概率——这正是本文件
 *   为 `$HOME` 那条记过的教训。
 * - **`undecided`** 于是没有了消费者。判不出它属于那三类，就说明它不属于，放行。
 *
 * ## 组合命令不需要单独处理
 *
 ...
 *
 * 这里原来的用途是配合「目标是 `.qy/`」判自我提权，那条规则已经删了（见 HARD_DENY
 * 上方的说明）。读类动词（`cat` / `type` / `Get-Content`）刻意不在其中。
 */
 ...
 *
 * 这里曾经有一条「用 shell 写 .qy/ 或 .agents/ = 自我提权」。两条理由让它站不住：
 *
...
export function decideCommand(command: string, ctx: PolicyContext): PolicyDecision {
   ...
   * 照样被抓到，不需要先判「这是不是单条命令」——上一版为此维护的那套组合符号
   * 扫描，存在的理由只是保护允许清单，而允许清单已经没有了。
   */
