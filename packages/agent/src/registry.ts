 ...
 *
 * ## 与青研魔盒的关系：机制照搬，类目清单不同
 *
 * 照搬的是**机制**：`category`（大类）+ `facet`（类内功能方向）+ `summary`（一句话用途）
 * 三层，注册期必填，缺一即失败（魔盒 `tool_registry.py` 的 `taxonomy()` 把这道闸放在
 * 取数时抛错，这里放在注册期，更早）。
 *
 * 不同的是**清单**：魔盒是七个内置（`contracts.py:47-62`：sheet / code / files / web /
 * knowledge / planning / session）加一个类外的 external。qywork 少一个 `sheet`——
 * 那是魔盒的业务核心（工作簿、表格），这个仓是编码 agent，一个表格工具都没有，
 * 建一个空类目就是 B5 的空壳。所以这里是**六个内置**，不是七个。
 *
 * `external` 两边都不算在内置里（魔盒原话：「EXTERNAL 是内置七类之外的自动归位类」）：
 * MCP 与插件的工具自动归它，因为它们的类目由第三方决定，混进内置分类学会让
 * 「文件与草稿」那一栏突然冒出别人家的工具。**它不是兜底桶**——内置工具漏标
 * 不会落进来，那种情况注册直接失败。
 *
 * 枚举顺序即界面呈现顺序。
 */
