 ...
 * ## 与参照物的一处差别，如实记着
 *
 * 参照物（`青研魔盒` 的 `run/progress.py`）在**下一个动作发出之前**判，
 * 拿候选动作跟周期首个动作比对，所以能早一轮拦住。这里改成**批次跑完之后**判，
 * 晚一轮。原因是形状不同：那边逐个动作调度，这里是整批下发，
 * 提前中断会在 transcript 里留下一条「有 tool_calls 但没有 tool 结果」的
 * assistant 消息——下一轮请求发给 provider 会直接 400。
 * 晚一轮的代价是多跑一个批次，比起烧满 max_steps 仍然是数量级的差别。
 */
