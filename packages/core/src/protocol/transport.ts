 ...
 * 同一批出的」，而它挡不住：那个数只在有人**记得**手动 +1 时才生效，没有任何东西
 * 强制。真实发生过的漂移（旧 sidecar 少一条 HTTP 路由）根本没动线上格式，
...
export interface ServerCapabilities {
  ...
  // 横跨用户同时开着的所有项目；插件虽然是全局的，但它的工具清单要和另外两份
  // 一起回才凑得齐「这个工作区上模型有哪些工具」。报在握手里等于「A 项目的编排
  // 显示在 B 项目上」，而且只有重连时才更新。改由 `/api/capabilities?ws=` 回答。
  /**
   * shell 命令有没有内核级边界。
   *
