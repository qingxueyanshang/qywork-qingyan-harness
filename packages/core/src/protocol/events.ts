 ...
 * 断流后正在重发。
 *
 * 两种形式都发这条事件（判据在 `agent/loop/attempt.ts` 的尝试循环）：正文未显示的同一份
...
export interface ToolStartedEvent {
   ...
   *
   * **必填。** 名字不在注册表里的调用根本走不到这里——`agent/loop/tool-wave.ts` 在编排波次之前
 ...
 * 唯一权威；名字不在表里的调用在 `agent/loop/tool-wave.ts` 里就被挡在执行链之外，永远不会
...
export interface DesktopTargetEvent {
  ...
  target: {
    ...
    app: string
    /** 本次占用已用过前台接管；后台读取不会撤回它，释放时随目标一起清空。 */
    foreground: boolean
  } | null
}
