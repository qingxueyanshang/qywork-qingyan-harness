...
`,
  },
  {
     ...
     * 这一行才能答出「这条会话跑在哪个根」。删了行，会话就永远打不开。
     * 「不留中间态」的办法不止删行一种——**行留着，但不出现在列表里**。
     *
...
`,
  },
  {
     ...
     * ## 为什么必须有这两列
     *
     * `recoverStaleRuns` **不能无差别扫全库的 running/queued**：一台机器上可以有
     * 好几个进程写同一个账本（两个工作区各一个 sidecar、开发态的热重载、
     * 终端里的 `qy exec`），无差别扫的话**后起的那个进程一启动，就把前一个正在跑
     * 的那一轮判成中断**，而那个进程还活着、还在往下写。实测撞到过：一条跑了
     * 40 步的 run 在第 27 次请求发出后 257 毫秒被判死，写入者是另一个刚起来的进程。
     *
     * 判「还活着」需要两个信号，缺一不可：
     *
...
`,
  },
  {
     ...
     *
     * ## 不能挂在 `ToolContext.state` 上
     *
     * 那张 map 是 **run 内的便签**（批级预算、计划快照也在里面，两者都必须每轮
     * 清零），挂上去「读过 x.ts」就每轮清零一次：模型上一轮读过、这一轮直接改，
 ...
 * 它就开始说谎，而且不会有人发现：真正决定迁移的是 `_migrations` 表，
