 ...
 * ## 为什么要有这一层
 *
 * 只有 `bun test`（免费、无网络、只测单元）和 `scripts/smoke-serve.ts`（真 key、
 * 五分钟、跑真模型）两档的话，**中间是空的**，而那道缝真的漏过东西：
 * `bun test` 看不见 serve 的装配，smoke-serve 要真 key 才跑、于是没人跑，
 * 一条早就不成立的断言在缝里躺了很久没人看见它红。
 *
