 ...
 * 这里不测。
 *
 * 钉的是两条**安全边界**，它们现在只存在于代码里，没有任何东西挡着后来的重构：
 *
...
describe('删一个插件', () => {
   ...
   * （`/api/plugins/..` → `/api/`），那条路由压根不匹配。反斜杠同理——WHATWG 对
   * 特殊 scheme 会把 `\` 归一成 `/`，于是变成两段、路由也不匹配。
   *
   * **留下来能到达处理器的是这两类**：id 里夹着 `..`（`..x` / `a..b`），
   * 以及百分号编码（`%2e%2e%2f`，pathname 不解码，它会原样进到 `join`）。
   * 断言分两层：夹着 `..` 的必须 400；两类都必须**没碰到插件目录外面的东西**。
   */
  test('id 里带 .. 的被拒，插件目录外面的东西一个都不会少', async () => {
