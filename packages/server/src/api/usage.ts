 ...
 * ## 为什么它到现在才有
 *
 * 账本（`usage_ledger`）一直在被正常写入，`usageTotals` / `usageBy` 也一直在，
 * 但**只有 `qy usage` 这个 CLI 在读**。于是界面上唯一能看到的用量是
 * 「当前会话的 runs 加起来」——会话一删就没了，也答不了「这个月花了多少」。
 * 这是 ARCHITECTURE §11 那张表里的第三种形状：两头都好，中间少一节。
 *
 * ## 口径与 CLI 完全一致
 *
 * 同一组函数、同一组参数，不在这里另算一遍。界面和命令行报出不同的数字，
 * 比其中一个报错要难查得多。
 */
