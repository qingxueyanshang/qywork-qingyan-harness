 ...
 * ## 为什么必须有这条命令
 *
 * 它是 `sandboxed` 与 `netGuarded` 的**唯一用户可达出口**。没有它，这两个值在
 * `packages/plugins` 之外一个消费者都没有，唯一的痕迹是 `PluginHost.start()` 里的
 * 一行 stderr——用户装完插件想知道「它加载了吗、被隔离了吗」，只能起一轮真实 run
 * 然后去日志里翻，而桌面外壳会把那些输出吞掉。
 *
