 ...
 * 插件进程隔离。
 *
 * ## 为什么必须做，而不是留一句「只装你信任的插件」
 *
 * 同进程加载下，插件的权限声明是**协作式**的：它约束的是通过宿主 API 的调用路径，
 * 挡不住插件直接 `import('node:fs')` 读走用户的 `~/.ssh`，也挡不住它
 * `import('node:child_process')` 起个反弹 shell。
 *
 * 文档里写「只装你信任的插件」是**用免责声明代替安全边界**。
 * 插件生态的意义就是装别人写的东西；要求用户先审计源码等于取消了这个生态。
 *
 * ## 隔离方式：子进程 + stdio JSON-RPC
 *
 * 不用 `worker_threads`：worker 与主线程共享同一个进程和同一份 `process.env`，
 * 宿主的 API Key 对它是直接可读的。子进程才能给一份洗过的环境。
 *
 * ## 这道边界**实测**挡住了什么、没挡住什么
 *
 * 挡住的：
 ...
 *
 * **两者都不成立时**（bun、旧版 node、用户指定了运行时），插件进程就是一个
 * 普通的 Node/Bun 进程：它能 `import('node:fs')` 读主目录、`import('node:net')`
 * 开套接字。那时候 **manifest 权限管的是「通过宿主做事」，不是「插件能做什么」**。
 *
 * 就算两者都成立，也仍然有一条**定义上**的缺口：拿到 `process:exec` 的插件
 * 能起子进程，能起子进程就能跑 curl。所以那种情况下 `netGuarded` 如实报 false。
 * 而且出网闸是**进程内的拆除不是内核边界**——准确的说法是
 * 「把顺手联网变成必须刻意绕」，不是「插件绝对上不了网」。
 *
 * 这段话的措辞是**修过的**。之前写的是「插件拿不到 fs / net / child_process」，
 * 那是错的而且错得危险：它让权限清单看起来像一道沙箱，于是用户会据此判断
 ...
 * ## 权限在宿主侧强制，不在插件侧
 *
 * 插件请求宿主能力时，宿主按 manifest 声明的权限校验，插件的自我声明不作数。
 * 校验表在本文件末尾的 `requiredPermission()`，未登记的方法一律拒绝。
 */
 ...
 * 权限校验。
 *
 * **在宿主侧执行**，而不是信任插件的自我声明。方法名到权限的映射写死在这里，
