...
describe('沙箱实测：只声明 workspace:read 的插件', () => {
  ...
  /**
   * 这条曾经**故意断言「没挡住」**，并且写着「哪天真堵上了，这条会红」。
   * 出网闸落地那天它如约变红了——现在断言的是相反的事实。
   *
   * 保留这段来历不是怀旧：它是这个项目里唯一一条**预告了自己会失效**
   * 并且真的按预告失效的测试，说明「把已知缺口写成会红的断言」这个做法有用。
   */
  test('直接开套接字已被挡住 —— 这是出网闸落地后的新事实', async () => {
 ...
 * 逃逸路径逐条实测。
 *
 * ROADMAP §21-1 里那张表是探出来的，但探测代码当时没留在仓库里——
 * 于是「挡住了」这件事只活在一份文档里，没有任何东西盯着它。
