 ...
 *
 * 之前插件宿主默认拿 `process.execPath` 当运行时。开发时那是 `bun`，一切正常；
 * **而发布产物是 Bun 编译出来的单文件二进制，`process.execPath` 就是 `qy.exe` 本身**。
 * 拿它去跑 `qy <插件入口>` 的结果是「未知命令」+ 用法说明，插件启动即退出。
 *
 * 也就是说：插件在开发机上一直好好的，装了包的用户那里**一个都起不来**，
 ...
 * ## 沙箱：能做到多少就说多少
 *
 * Node 的权限模型（`--permission`，20/22 上叫 `--experimental-permission`）
 * 能把文件系统、子进程、worker、原生插件关起来。实测：
 *
 ...
 *
 * 最后一行曾经写的是「仍然可用」，那是当时的事实。现在网络那一面由
 * `netguard.ts` 单独补上，但它是**进程内的拆除不是内核边界**，
