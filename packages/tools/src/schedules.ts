 ...
 * 每次触发都留下一个可以点开的会话。
 *
 * ## 为什么这一份在 `tools` 而不在 `runtime`
 *
 * 因为模型侧的三个工具（本文件下半段）必须在这里：工具都在 `tools` 包，
 * 而 `tools`(L3) 不许依赖 `runtime`(L5)——依赖只能朝更底层走
 * （`scripts/dependency-graph.test.ts`）。
 *
 * 下沉不需要付任何代价：这份代码对 `runtime` 的唯一依赖是 `configDir()`，
 * 而它本身就是 `globalScopeRoot()` 的一层转手（`runtime/config.ts:178`），
 ...
 * 在此之前只有人能排定时任务，模型知道这个能力存在却没有任何办法用它。
 *
 * 三条共同约束：
 *
...
export const createScheduleTool: ToolSpec = {
  ...
  async fn(args, ctx) {
    ...
    const draft: Schedule = {
      ...
      // 这里少一个字段意味着模型没想清楚跑在什么时候，悄悄补一个 9:00 的话
