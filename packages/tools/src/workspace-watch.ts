 ...
 * 补挂之前写进去的标记没有事件。
 * Bun 对同一个 watcher 在约 2 ms 内与上一条类型相同或路径相同的事件不交付（Linux 与 Windows 实测，
 * 2026-09-25：连续删两个文件，后一条多数丢失）。标记每次新建而不改写已有文件：新建产生 rename 与
 * change 两类事件，至多丢一条；改写只有 change，紧跟在别的修改之后时一条都收不到。
 * 标记的 rename 同样占这 2 ms，与收尾同时发生在另一个窗口里的删除可能因此丢失。
 *
