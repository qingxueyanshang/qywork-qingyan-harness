 ...
 * ## 先读头，只对真超标的动手
 *
 * **不能读到图就重编码。** 实测（2026-08，photon 0.3.4）：一张 1440×900 的网页截图
 * 485 KB，本来就在上限内，重编码成 PNG 之后是 **1174 KB——大了 2.4 倍**。
 * 浏览器与截图工具的 PNG 编码器比这个库好得多，原样通过才是对的。
 *
 ...
 * ## 超标了才解码，输出取更小的那个
 *
 * 同一次实测：3200×2400 的 2764 KB 缩到 1568 长边之后，PNG 是 1143 KB、
 * JPEG(82) 是 88 KB。差一个数量级，**必须两个都编一遍取小的**——
 * 只出 PNG 等于白缩。
 *
...
export async function shrinkImage(
  ...
  try {
    ...
    // 两种编码都试，取小的。实测差一个数量级——只出 PNG 等于白缩。
