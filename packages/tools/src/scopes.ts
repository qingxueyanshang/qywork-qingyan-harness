 ...
 * 用户层就能悄悄替换掉系统自己的行为。cc-haha 的顺序同样是
 * `managed → user → project` 且先认领的赢，而 `managed` 是不可写的企业策略目录。
 *
 ...
 *
 * `.agents/` 是跨客户端约定（agentskills.io），Claude Code 一系同时读它。
 * 别的 CLI 读不到我们的 `~/.qywork/`，但读得到工作区里这一份——
 * 「换个 CLI 也能用」这件事只有这一层做得到。**但它不是万能**：
 * pi 用 `.pi/skills`，不读 `.agents`。准确说法是「对 claude / codex 这一系可移植」。
 *
 * ## 解析规则只能有一份
 *
