 ...
 * 技能。
 *
 * **能建、能导、能删，不能在这里改正文。** 一个技能最少就是 `<目录>/SKILL.md`，建一个用一个表单就
 * 够。改正文不在这里：技能目录里可以带脚本和附件，在网页上编辑一个目录需要一整套文件管理器，那是
 * 编辑器该干的事。所以卡片上给出目录名，要改就去那儿改。
 *
 * **说明是必填。** 扫描器对没有 `description` 的目录是**静默跳过**的（那种技能装了也不会被用到）。
 * 表单不拦的话，建完刷新列表里没有，而用户完全无从知道为什么。
 *
 * **按层分列。** 「这个技能是跟着这个仓库走的，还是全局都生效的」是这一页要回答的第一个问题。
