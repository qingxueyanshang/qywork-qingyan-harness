...
describe('工具图片回放', () => {
  ...
  /** 生成的产物只显示路径，点了在右侧预览里打开；不收进工具组，否则被折叠看不见。 */
  test('声明 files: open 的结果列出可点的产物路径，点了打开右侧预览', async () => {
    const store = await import('../lib/store/index.ts')
          ...
          transcript: [
            {
              ...
              args: { path: 'a.ts' },
              status: 'success',
              outcome: { status: 'success', executed: true, message: '读取 a.ts' },
            },
            {
              id: 'tool-generated',
              kind: 'tool',
              text: '',
              toolName: 'generate_image',
              action: { kind: 'write', objectLabel: '图片', target: null },
              args: { prompt: '一只猫' },
              status: 'success',
              outcome: {
                status: 'success',
                ...
                message: '生成 2 张',
                fileChanges: [
                  { path: 'generated/a.png', changeType: 'created' },
                  { path: 'generated/a-2.png', changeType: 'created' },
                ],
    ...
    try {
      const links = Array.from(host.querySelectorAll<HTMLButtonElement>('.tool-files .tool-file'))
      expect(links.map((l) => l.textContent)).toEqual(['generated/a.png', 'generated/a-2.png'])
      expect(host.querySelector('.tool-images')).toBeNull()
      const click = (links[1] as unknown as { $$click?: (e: Event) => void }).$$click
      click?.call(links[1], new MouseEvent('click'))
      expect(store.openFile()).toBe('generated/a-2.png')
    } finally {
      dispose()
      host.remove()
      store.setOpenFile(null)
