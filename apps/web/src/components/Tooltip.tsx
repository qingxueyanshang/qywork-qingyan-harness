...
const GAP = 6
/**
 * 悬停到出现之间的等待。原生 `title` 约 1s，慢到让人以为没有提示；
 * 短于 200ms 则划过工具栏时一路连闪。
 */
const DELAY = 350
 ...
 * ## 不用原生 title
 *
 * `title` 的气泡由系统绘制，CSS 够不着：字体、圆角、配色、延迟都不受控，
 ...
 * ## 单实例 + fixed 定位
 *
 * 触发点大多在 `overflow: hidden` 的容器里（输入区工具栏、侧栏列表、文件树），
 * 用 `::after` 挂在触发元素上会被祖先裁掉。fixed 脱离所有滚动容器，
 * 顺带能在贴近视口下沿时翻到上方。
 *
 * ## 边界
 *
