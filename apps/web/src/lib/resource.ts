 ...
 * ## 为什么不直接写 `data()`
 *
 * Solid 的 `resource()` 和 `resource.latest` 在出错时都是 `throw err`，指望调用方
 * 外面有 `ErrorBoundary` 接住。这个应用一个都没有——抛出去没人接，那一帧的更新
 * 半途中断，页面停在残缺状态。**更坏的是它把已经写好的错误界面变成了死代码**：
 * `<Show when={data()} fallback={<LoadState error={data.error} …/>}>` 这种写法里，
 * `when` 先抛，`fallback` 永远轮不到，于是「接口挂了显示原因 + 重试」这条路
 * 从来没有跑通过。
 *
