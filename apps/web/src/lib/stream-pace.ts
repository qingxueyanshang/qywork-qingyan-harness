 ...
 *   不至于越拖越远。稳态下它等于「显示落后到达约 1.2 秒」，谷正是靠这段落后填平的。
 * - `MAX_CHARS` 是硬顶：**没有它，一次大突发照样一帧糊出去**——
 *   软目标是「剩余 ÷ 24」，剩余一万字时那也是一次 417 字。
 *
 * ## 每档不设下限
 *
 * **不要给每档加保底字数。** 上游按批转发是常态（2026-08-20 实测一家中转站：
 * 每约 960ms 给 37 字一批），下限 6 字会让一批在 350ms 内排空、剩下 650ms 无字可放
 * ——按该序列模拟，63% 的档是空的，一秒一次停顿。没有下限时 `ceil(37/24)` 是 2 字一档，
 * 正好铺到下一批到达，空档降到 0。`ceil` 已保证只要还有字就至少放一个。
 *
 * ## 终态一律不排队
 *
 * run 结束、报错、被中断，都要立刻把积压全放出去。原因不是好看：
 * 运行读数条与错误卡读的是同一份 transcript，让它们看到一份
...
export function sliceSize(remaining: number): number {
  ...
  return Math.min(MAX_CHARS, Math.ceil(remaining / DRAIN_TICKS))
}
